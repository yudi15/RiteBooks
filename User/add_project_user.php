...
<html
  ...
  <head>
    ...
    <title>Assign Project--RiteBooks</title>
    ...
    <div class="form-floating form-floating-outline">
        <select id="select2Basic" class="select2 form-select share-project-select" data-allow-clear="true">
            ...
            if ($result->num_rows > 0) {
                // Output the logged-in user's data
                $row = $result->fetch_assoc();
                $fullName = htmlspecialchars($row['first_name'] . " " . $row['last_name']);
                echo "<option data-name='$fullName' value='{$row['id']}' selected>$fullName</option>";
            } else {
                echo "<option>No user found</option>";
            }
            ?>
        </select>
				<label for="select2Basic">User</label>
		</div>
			</div>
